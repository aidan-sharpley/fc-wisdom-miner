...
from config.settings import THREADS_DIR, _file_lock
...
def get_thread_dir(thread_key: str) -> str:
    ...
    path = os.path.join(THREADS_DIR, sanitized_key)
...
def list_threads() -> List[str]:
    ...
    if not os.path.isdir(THREADS_DIR):
        logger.debug(f"Base directory {THREADS_DIR} does not exist")
        return []

    try:
        directories = []
        for item in os.listdir(THREADS_DIR):
            item_path = os.path.join(THREADS_DIR, item)
            if (
                os.path.isdir(item_path)
                and item not in ("__pycache__", ".git", ".pytest_cache")
        ...
        logger.error(f"Failed to list threads in {THREADS_DIR}: {e}")
        return []
...
def cleanup_old_files(
    directory: str, max_age_days: int = 30, pattern: str = "*.tmp"
) -> int:
