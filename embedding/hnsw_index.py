...
from config.settings import (
    ...
    HNSW_MAX_ELEMENTS, HNSW_INDEX_NAME
)
...
class HNSWIndex:
    ...
    def __init__(self, thread_dir: str, dimension: int = 384):
        ...
        self.metadata_path = os.path.join(thread_dir, f"{HNSW_INDEX_NAME}.metadata.json")
    ...
    def _load_or_create_index(self):
        ...
        try:
            if os.path.exists(self.index_path) and os.path.exists(self.metadata_path):
                self._load_index()
                logger.info(f"Loaded HNSW index with {self.metadata['num_elements']} elements")
            else:
    ...
    def search(self, query_embedding: np.ndarray, k: int = 10) -> Tuple[List[str], List[float]]:
        ...
        try:
            ...
            # Search
            indices, distances = self.index.knn_query(query_embedding, k=min(k, self.metadata['num_elements']))
            ...
            logger.debug(f"HNSW search completed in {search_time:.3f}s, found {len(post_hashes)} results")
