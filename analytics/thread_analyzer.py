...
from utils.text_utils import extract_keywords, get_text_statistics, detect_language, calculate_readability_score
...
class ThreadAnalyzer:
    ...
    def analyze_thread(self, posts: List[Dict], force_refresh: bool = False) -> Dict[str, Any]:
    ...
    def _analyze_participants(self, posts: List[Dict]) -> Dict[str, Any]:
        ...
        for post in posts:
            ...
            author_data['first_post_position'] = min(author_data['first_post_position'], position)
            author_data['last_post_position'] = max(author_data['last_post_position'], position)
        ...
        most_active = max(processed_authors.items(), key=lambda x: x[1]['post_count']) if processed_authors else ('unknown', {})
        ...
        return {
            ...
            'avg_posts_per_author': sum(post_counts) / len(post_counts) if post_counts else 0,
    ...
    def _analyze_content(self, posts: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'post_length_stats': {
                ...
                'median': sorted(post_lengths)[len(post_lengths)//2] if post_lengths else 0
            },
            ...
            'content_diversity': len(set(thread_keywords)) / len(thread_keywords) if thread_keywords else 0
        }
    ...
    def _analyze_temporal_patterns(self, posts: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            peak_day = max(daily_posts.items(), key=lambda x: x[1]) if daily_posts else (None, 0)
            ...
            return {
                ...
                'posting_frequency': {
                    ...
                    'max_posts_per_day': max(daily_posts.values()) if daily_posts else 0,
                    'avg_posts_per_active_day': sum(daily_posts.values()) / len(daily_posts) if daily_posts else 0
                }
            }
    ...
    def _analyze_topics(self, posts: List[Dict]) -> Dict[str, Any]:
        ...
        for i, word1 in enumerate(keywords[:20]):  # Limit for performance
            ...
            for j, word2 in enumerate(keywords[:20]):
                if i != j and (word1 in all_content.lower() and word2 in all_content.lower()):
        ...
        return {
            ...
            'keyword_diversity': len(set(keywords)) / len(all_content.split()) if all_content else 0,
            'estimated_topics': min(10, len(topic_words))  # Rough estimate of topic count
        }
    ...
    def _analyze_interactions(self, posts: List[Dict]) -> Dict[str, Any]:
        ...
        for i in range(1, len(posts)):
            ...
            if current_author != prev_author and current_author != 'unknown' and prev_author != 'unknown':
        ...
        return {
            ...
            'total_interactions': sum(
                sum(interactions.values()) for interactions in author_interactions.values()
            )
        }
    ...
    def _calculate_statistics(self, posts: List[Dict]) -> Dict[str, Any]:
        ...
        # Position statistics
        positions = [post.get('global_position', 0) for post in posts if post.get('global_position')]
        
        return {
            'total_posts': total_posts,
            'total_characters': total_characters,
            'avg_characters_per_post': total_characters / total_posts if total_posts else 0,
            ...
            'posts_per_page': {
                ...
                'avg': sum(page_distribution.values()) / len(page_distribution) if page_distribution else 0
            },
            'thread_health': {
                'completeness': len(positions) / total_posts if total_posts else 0,
                'consistency': 1.0 if positions == list(range(1, len(positions) + 1)) else 0.8
            }
        }
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            'overview': {
                ...
                'participants': analytics.get('participants', {}).get('total_participants', 0),
                'pages': analytics.get('metadata', {}).get('total_pages', 0),
                'primary_language': analytics.get('content', {}).get('primary_language', 'unknown')
            },
            'activity': {
                'most_active_author': analytics.get('participants', {}).get('most_active_author', {}),
                'avg_posts_per_author': round(analytics.get('participants', {}).get('avg_posts_per_author', 0), 1),
                'thread_duration_days': analytics.get('temporal', {}).get('thread_duration_days', 0)
            },
            'content_insights': {
                'primary_keywords': analytics.get('topics', {}).get('primary_keywords', [])[:5],
                'avg_post_length': round(analytics.get('content', {}).get('post_length_stats', {}).get('avg', 0)),
                'readability_score': round(analytics.get('content', {}).get('avg_readability', 0), 1)
            },
            'interaction_level': {
                'total_interactions': analytics.get('interaction', {}).get('total_interactions', 0),
                'participation_diversity': analytics.get('participants', {}).get('participation_distribution', {})
            }
        }
